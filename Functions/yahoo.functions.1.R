...
# yahoo.recurse(ticker) ---------------------------------------------------

yahoo.recurse <- function(ticker){
...
yahoo.scrape <- function(ticker){
    stopifnot(is.character(ticker))
    source("yahoo.recurse.R")
    source("check.pkgs.R")
    if (!check.pkgs("rvest")) library(rvest)
    if (!check.pkgs("plyr")) library(plyr)
    yahoo.results        <- lapply(ticker, yahoo.recurse)
    names(yahoo.results) <- ticker
    yahoo.results
}
